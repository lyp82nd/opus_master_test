// WAV audio loader and writer. WAV 
// 音频加载器和编写器
// Public domain.
...
// 然后，您可以像处理其他头文件一样,在程序的其他部分中包含这个#文件。要读取音频数据，请执行以下操作:
//
//     drwav wav;
...
// 您还可以使用 drwav _ open ()为您分配和初始化加载程序:
//
...
//如果您只想在一个操作中快速打开并读取音频数据，您可以执行以下操作
...
// 上面的例子使用的 API 版本将音频数据转换为一致的格式(本例中是32位带签名的 PCM) 
// 但是你仍然可以输出内部格式的音频数据(参见下面支持的格式注释) :
//
...
// 您还可以读取音频数据的原始字节，如果 dr _ wav 不支持特定的数据格式，这将非常有用:
//
...
// Dr _ wav 无缝地支持索尼 Wave 64格式。解码器将自动检测到它。                  
//
//
...
// Dr _ WAV 也可以用来输出 WAV 文件。这当前不支持压缩格式。
// 要使用它，请查看 drwav _ open _ write ()、 drwav _ open _ file _ write ()等。
// 使用 drwav _ write ()编写示例，或者使用 drwav _ write _ raw ()编写“ data”块中的原始数据。
// 
...
// 默认的 read 函数不执行任何数据转换。使用 drwav _ read _ f32()读取音频数据并将其转换为 IEEE 32位浮点样本，
// 使用 drwav _ read _ s32()读取32位有符号 PCM 的样本，使用 drwav _ read _ s16()读取16位有符号 PCM 的样本。
// 经过测试和支持的内部格式包括:
// 
//   - Unsigned 8-bit PCM
//   - Signed 12-bit PCM
...
// Dr _ WAV 将尽可能读取.WAV 文件，即使它不严格遵守 WAV 格式
...
//返回值小于 bytesToRead 表示流的结束。不要从这个回调函数返回 _ not _ return，直到填满整个 bytesToRead 或者到达流的末尾为止。
typedef size_t (* drwav_read_proc)(void* pUserData, void* pBufferOut, size_t bytesToRead);

// Callback for when data is written. Returns value is the number of bytes actually written.
// 数据写入时的回调。返回值是实际写入的字节数。
//
...
//需要查找数据时的回调。返回查找是否成功。 Callback for when data needs to be seeked.
//
// pUserData [in] The user data that was passed to drwav_init(), drwav_open() and family.
...
typedef struct
{
    // 格式标签=1The format tag exactly as specified in the wave file's "fmt" chunk. This can be used by applications
    // that require support for data formats not natively supported by dr_wav.
    drwav_uint16 formatTag;

    // 声道数The number of channels making up the audio data. When this is set to 1 it is mono, 2 is stereo, etc.
    drwav_uint16 channels;

    // 采样率The sample rate. Usually set to something like 44100.
    drwav_uint32 sampleRate;

    // 数据传输速率Average bytes per second. You probably don't need this, but it's left here for informational purposes.
    drwav_uint32 avgBytesPerSec;

    // 块对齐=通道数*采样位数/8  Block align. This is equal to the number of channels * bytes per sample.
    drwav_uint16 blockAlign;

    //采样位数 Bit's per sample.
    drwav_uint16 bitsPerSample;

    // 扩展数据”的大小。这个参数在内部使用，主要用于数据的验证等目的，但在此处被保留出来，以便提供更多信息The size of the extended data. Only used internally for validation, but left here for informational purposes.
    drwav_uint16 extendedSize;

    // 每个样本点在音频文件中实际使用的比特位数。在WAVE_FORMAT_EXTENSIBLE格式下，该参数始终向上取整至最接近8的倍数的值。这个参数主要用于提供关于音频文件格式的信息，方便用户或者程序在处理或者转换音频文件时，能够准确地应用这些格式相关的细节。The number of valid bits per sample. When <formatTag> is equal to WAVE_FORMAT_EXTENSIBLE, <bitsPerSample>
    // is always rounded up to the nearest multiple of 8. This variable contains information about exactly how
    // many bits a valid per sample. Mainly used for informational purposes.
    drwav_uint16 validBitsPerSample;

    // 通道掩码”，它指示了在声音数据中，哪些声道是被使用的，目前并没有被应用到实际中The channel mask. Not used at the moment.
    drwav_uint32 channelMask;

    // 子格式，它是用来指定PCM数据编码格式的，也就是数据的存储方式、精度和采样率等信息，这个参数会影响到Wave文件实际的内容，比如说数据的解码方式、可播放性和兼容性等。在Wave文件的文件头中，这个参数被明确地指定了，并且也必须和实际的数据格式保持一致，否则将会导致错误或播放失败。因此，这个参数的作用是确保了Wave文件的正确性和可靠性，以及兼容性和可播放性。The sub-format, exactly as specified by the wave file.
    drwav_uint8 subFormat[16];
} drwav_fmt;

typedef struct
{
    //当 dr_wav.h 需要读取更多的音频数据时，会调用 onRead 函数，并传递以下四个参数：
    //    pUserData：指向用户自定义数据的指针，该指针由 drwav_open() 或 drwav_open_memory() 函数传递给库。
    //    buffer：指向用于存储读取数据的缓冲区的指针。
    //    bytesToRead：需要读取的字节数。
    //    pBytesRead：实际读取的字节数。
    //    用户自定义的回调函数应该从 pUserData 中获取所需的数据，并将最多 bytesToRead 个字节的数据读入 buffer 中。它应该将实际读取的字节数写入 pBytesRead 指针中，以便知道实际读取了多少字节的数据。
    drwav_read_proc onRead;

    //pUserData：是一个指向用户自定义数据的指针，这个指针在调用 drwav_init_write() 时可以传递给库 
    //pData：表示要写入的数据缓冲区的指针 
    //bytesToWrite：表示要写入的数据大小，单位为字节（byte）。
    //当需要写入数据时，dr_wav.h 库会调用用户实现的 onWrite 函数，并将上述三个参数传递给它。在该函数中，用户需要将 pData 指向的数据写入到 WAV 文件中，并返回实际写入的字节数。如果写入失败，则需要返回一个小于 bytesToWrite 的值。A pointer to the function to call when data needs to be written. Only used when the drwav object is opened in write mode.
    drwav_write_proc onWrite;

    //pUserData：是一个指向用户自定义数据的指针
    //offset：表示要跳转的偏移量，单位为字节（byte）
    //origin：表示跳转的起始位置
    //当需要进行跳转操作时，dr_wav.h 库会调用用户实现的 onSeek 函数，并将上述三个参数传递给它。在该函数中，用户需要根据 offset 和 origin 参数计算出正确的跳转位置，并将文件指针移动到该位置。最后，函数需要返回实际进行了多少字节的跳转，如果跳转失败则返回一个负数。A pointer to the function to call when the wav file needs to be seeked.
    drwav_seek_proc onSeek;

    // 通常情况下，用户可以使用这个指针来传递一些额外的数据或上下文信息给回调函数，例如文件句柄、缓冲区、解码器等。这个指针的具体用法取决于用户的需求和实现方式。The user data to pass to callbacks.要传递给回调的用户数据
    void* pUserData;


    //用于指定打开的 WAV 文件的容器格式。这个参数在打开 WAV 文件时传递给 drwav_open() 函数。
    //container 参数有以下几种取值：
    //    drwav_container_riff：表示使用 RIFF 格式的 WAV 文件。
    //    drwav_container_w64：表示使用 W64 格式的 WAV 文件。
    //    默认情况下，drwav_open() 函数会自动检测 WAV 文件的容器格式，并选择相应的解析器进行解析。因此，在大多数情况下，不需要显式地指定 container 参数。 Whether or not the WAV file is formatted as a standard RIFF file or W64.
    drwav_container container;


    // drwav_fmt fmt 参数用于指定 WAV 文件的格式信息，包括采样率、位深度、通道数等。这些信息在解析或写入 WAV 文件时非常重要，因为它们决定了音频数据的格式和存储方式。具体来说，drwav_fmt 结构体包含以下成员：
    //uint16_t formatTag：指定编码格式，如 PCM、ADPCM 等。
    // uint16_t channels：指定通道数，可以是单声道或立体声等。
    //uint32_t sampleRate：指定采样率，即每秒采样次数。
    //uint32_t avgBytesPerSec：指定每秒的平均字节数，计算公式为采样率 x 位深度 x 通道数 / 8。
    //uint16_t blockAlign：指定数据块对齐单位大小，计算公式为位深度 x 通道数 / 8。
    //uint16_t bitsPerSample：指定位深度，即每个采样点占用的比特数。
    //通过设置这些参数，我们可以指定 WAV 文件的格式信息，从而正确地读取或写入音频数据Structure containing format information exactly as specified by the wav file.
    drwav_fmt fmt;

    // 采样率The sample rate. Will be set to something like 44100.
    drwav_uint32 sampleRate;

    //通道数 The number of channels. This will be set to 1 for monaural streams, 2 for stereo, etc.
    drwav_uint16 channels;

    // 采样位数The bits per sample. Will be set to somthing like 16, 24, etc.
    drwav_uint16 bitsPerSample;

    //采样位数换成字节 The number of bytes per sample.
    drwav_uint16 bytesPerSample;

    //  参数用于存储实际的编码格式信息Equal to fmt.formatTag, or the value specified by fmt.subFormat if fmt.formatTag is equal to 65534 (WAVE_FORMAT_EXTENSIBLE).
    drwav_uint16 translatedFormatTag;

    //组成音频数据的样本总数。使用 < total SampleCount > * < bytesPerSample > 计算保存整个音频数据所需的缓冲区大小。 The total number of samples making up the audio data. Use <totalSampleCount> * <bytesPerSample> to calculate
    // the required size of a buffer to hold the entire audio data.
    drwav_uint64 totalSampleCount;


    // 数据块的大小，通常用字节表示。在 WAV 文件中，数据块存储着音频数据的实际内容，其大小取决于采样率、位深度、通道数等参数，以及文件中实际包含的音频数据量。The size in bytes of the data chunk.
    drwav_uint64 dataChunkDataSize;
    
    // 数据块第一个字节在流中的位置。这用于进行跳转操作，当需要从文件中读取或写入音频数据时，需要知道该数据块在文件中的位置。这个位置信息可以通过解析 WAV 文件头部得到。而当需要进行跳转操作时，也需要知道数据块在文件中的位置，以便准确地定位到要跳转的位置。The position in the stream of the first byte of the data chunk. This is used for seeking.
    drwav_uint64 dataChunkDataPos;

    // 数据块中剩余的字节数”。在WAV文件中，数据块通常包含音频数据。该数据块中存储的字节数取决于音频数据的长度和格式。当需要从数据块中读取音频数据时，需要知道数据块中还有多少字节尚未被读取，以便准确地读取剩余的数据。而当需要写入音频数据时，也需要知道数据块中还有多少空间可用，以便避免写入超出数据块大小的数据。The number of bytes remaining in the data chunk.
    drwav_uint64 bytesRemaining;


    //在打开解码器之前，先将 WAV 文件数据拷贝到一个预先分配好的缓冲区中，然后将该缓冲区的地址传递给drwav_open_memory()函数即可。这样做的好处是可以避免使用动态内存分配函数，从而减少了内存管理的复杂 A hack to avoid a DRWAV_MALLOC() when opening a decoder with drwav_open_memory().
    drwav__memory_stream memoryStream;
    drwav__memory_stream_write memoryStreamWrite;

    // 压缩格式的通用数据，在 WAV 文件中，有一些音频编码格式采用了块压缩的方式来减小文件大小。这些格式包括ADPCM、MS ADPCM、IMA ADPCM等。对于这些格式，解码器需要一些额外的信息才能正确地解压缩音频数据。这些额外的信息被称为“通用数据”，它们与特定的压缩格式无关，可以在多个压缩格式之间共享Generic data for compressed formats. This data is shared across all block-compressed formats.
    struct
    {
        drwav_uint64 iCurrentSample;    // 当我们读取 WAV 文件中的数据块时，需要跟踪我们已经读取了多少个样本，并且需要确保我们不会尝试读取超过文件中实际存在的样本数。The index of the next sample that will be read by drwav_read_*(). This is used with "totalSampleCount" to ensure we don't read excess samples at the end of the last block.
    } compressed;
    ...
    struct
    {
        drwav_uint32 bytesRemainingInBlock;//块中剩下的字节
        drwav_int32  predictor[2];
        drwav_int32  stepIndex[2];
        drwav_int32  cachedSamples[16]; //在解码期间，样本存储在此缓存区中 Samples are stored in this cache during decoding.
        drwav_uint32 cachedSampleCount;//缓存区内样本数量
    } ima;
} drwav;


// 初始化一个预先分配好内存的 drwav 对象，用户需要先分配好内存空间，并将其传递给函数来初始化这个对象。这种方式可以避免在运行时动态分配内存，提高代码效率和稳定性Initializes a pre-allocated drwav object.
//drwav_init 函数用于初始化一个 drwav 对象。它需要以下参数：
//pWav：指向预先分配的 drwav 结构体的指针，该结构体将被初始化。
//onRead：onRead(pUserData, riff, sizeof(riff))就是从pUserData将sizeof(riff)个字节读取给 riff。
//onSeek：一个回调函数指针，用于在输入流中定位到特定位置。当解析器需要跳过一些数据时，或者需要重新读取之前读取的数据时，将调用此函数。
//pUserData：一个指向用户定义的数据的指针，它将被传递给回调函数。
//该函数返回一个 drwav_bool32 类型的值，如果初始化成功则返回 DRWAV_TRUE，否则返回 DRWAV_FALSE。初始化成功后，用户就可以使用 drwav 对象来读取和处理 WAV 文件了
...
// drwav 结构体是用于处理 WAV 文件的数据结构体，它保存了文件的相关信息，如采样率、通道数、总样本数等。在使用 drwav 结构体处理完一个 WAV 文件后，需要使用 drwav_uninit() 来释放这个结构体所使用的内存，并将结构体内的字段重置为默认值Uninitializes the given drwav object.
//
...
// 读取一段音频数据并将其转换为带签名的16位 PCM 样本。返回实际读取的样本数。如果返回值小于 < samplesToRead > ，则表示已到达文件的末尾。Reads a chunk of audio data and converts it to signed 16-bit PCM samples.
...
// 会尝试打开指定的 WAV 文件，并读取文件头信息，将这些信息存储在 drwav 结构体中。如果成功打开并读取了文件头，则返回 DRWAV_TRUE=1；否则返回 DRWAV_FALSE=0。Helper for initializing a wave file using stdio.
//
...
#ifndef DR_WAV_NO_STDIO
// 在一个操作中打开和解码一个 wav 文件Opens and decodes a wav file in a single operation.
...
typedef struct
{
    ...
    drwav_uint64 sizeInBytes;//记录了一个wav文件的数据块大小，表示要从文件中读取的字节数

    // RIFF = 2 byte 对齐alignment.
    // W64  = 8 byte 对齐alignment.如果不为0则手动添加填充字节使其可被2/8整取余
    unsigned int paddingSize;

} drwav__chunk_header;

//这个函数用了两次，一次读取了fmt块，一次读取了date块，
...
{
    if (container == drwav_container_riff) {//RIFF格式
        //pUserData第一次进入函数指向第12个字节pHeaderOut->id.fourcc用于存储 WAV 文件的标识符，应该为 "WAVE"
        //第二次进入pUserData指向36字节
        if (onRead(pUserData, pHeaderOut->id.fourcc, 4) != 4) {
            return DRWAV_FALSE;
        }
        unsigned char sizeInBytes[4];//用于表示数据块大小的参数
        if (onRead(pUserData, sizeInBytes, 4) != 4) {
            return DRWAV_FALSE;
        }

        pHeaderOut->sizeInBytes = drwav__bytes_to_u32(sizeInBytes);//转换为一个数
        pHeaderOut->paddingSize = (unsigned int)(pHeaderOut->sizeInBytes % 2);
        //pRunningBytesReadOut进入函数时指向12字节
        *pRunningBytesReadOut += 8;//=20字节，记录的就是读取puserdate的读取位置
    } else {//W64格式
        ...
        pHeaderOut->paddingSize = (unsigned int)(pHeaderOut->sizeInBytes % 8); //如果取余不为零则需要加填充字节
        pRunningBytesReadOut += 24;//记录的就是读取puserdate的读取位置是64字节
    }
...
{
    drwav_uint64 bytesRemainingToSeek = offset;//offset=header.sizeInBytes + header.paddingSize就是跳过所有格式信息的字节
...
static drwav_bool32 drwav__read_fmt(drwav_read_proc onRead, drwav_seek_proc onSeek, void* pUserData, drwav_container container, drwav_uint64* pRunningBytesReadOut, drwav_fmt* fmtOut)
{
    ...
    // 跳过无用的内容块Skip junk chunks.，符合junk格式的块就跳过
    if ((container == drwav_container_riff && drwav__fourcc_equal(header.id.fourcc, "JUNK")) || 
        (container == drwav_container_w64 && drwav__guid_equal(header.id.guid, drwavGUID_W64_JUNK))) 
    {
        if (!drwav__seek_forward(onSeek, header.sizeInBytes + header.paddingSize, pUserData)) {
            return DRWAV_FALSE;
        }
        *pRunningBytesReadOut += header.sizeInBytes + header.paddingSize;//更新读取位置
    ...
    unsigned char fmt[16];//记录wav文件格式,16字节
    
    if (onRead(pUserData, fmt, sizeof(fmt)) != sizeof(fmt)) {
        return DRWAV_FALSE;
    }
    *pRunningBytesReadOut += sizeof(fmt);//位置更新至36字节

    fmtOut->formatTag      = drwav__bytes_to_u16(fmt + 0);//2个字节的数据转换为一个数
    fmtOut->channels       = drwav__bytes_to_u16(fmt + 2);//2字节
    fmtOut->sampleRate     = drwav__bytes_to_u32(fmt + 4);//4字节
    fmtOut->avgBytesPerSec = drwav__bytes_to_u32(fmt + 8);//4字节
    fmtOut->blockAlign     = drwav__bytes_to_u16(fmt + 12);//2字节
    fmtOut->bitsPerSample  = drwav__bytes_to_u16(fmt + 14);//2字节
    ...
    if (header.sizeInBytes > 16) {//判断当前块的大小是否大于16字节（格式块最小为16字节），如果是，则认为该块包含一些额外的信息需要读取。
        printf("格式块大小大于16字节，有扩展信息");
        ...
        *pRunningBytesReadOut += sizeof(fmt_cbSize);
        ...
        //继续读取格式块后面的剩余信息。这些信息可能是扩展信息（只有在格式为DR_WAVE_FORMAT_EXTENSIBLE时才有），也可能是一些压缩格式的参数。
        ...
        //计算出剩余未读取的字节数，然后使用“onSeek”函数跳过这些字节，以保证下一个块的正确读取
        // 跳过任何剩余未读取的字节。对于W64文件格式，剩余字节的大小会根据Chunk大小定义。Seek past any leftover bytes. For w64 the leftover will be defined based on the chunk size.
...
drwav_bool32 drwav_init_file(drwav* pWav, const char* filename)
{
    ...
    if (fopen_s(&pFile, filename, "rb") != 0) {
        return DRWAV_FALSE;//返回值为 0 表示fopen_s(&pFile, filename, "rb")读取成功，pFile 指向已经打开的文件
    }
#else
...
drwav_bool32 drwav_init(drwav* pWav, drwav_read_proc onRead, drwav_seek_proc onSeek, void* pUserData)
{
    ...
    drwav_zero_memory(pWav, sizeof(*pWav));//将 pWav 指向的内存块中的每一个字节都设置为 0


    // WAV 文件的前四个字节应该是 "RIFF" 这个标识符。RIFF 是 Resource Interchange File Format 的缩写，是一种通用的文件格式标准，可以用于存储各种类型的数据，包括音频、视频、图像等。在 WAV 文件中，RIFF 标识符表示这是一个 RIFF 格式的文件，并且后面的数据块都遵循 RIFF 的规范。因此，如果一个文件不以 "RIFF" 开头，则不能被认为是一个有效的 WAV 文件。The first 4 bytes should be the RIFF identifier.
    unsigned char riff[4];
    if (onRead(pUserData, riff, sizeof(riff)) != sizeof(riff)) {
        return DRWAV_FALSE;    // Failed to read data.
    }

    // 对于 RIFF 格式的 WAV 文件，它们的前四个字节应该是 "RIFF"，而对于 w64 格式的 WAV 文件，则应该是 "riff"。
    //RIFF 和 riff 都表示 RIFF 格式的文件，只是大小写不同。RIFF 是标准的格式标识符，而 riff 则是一种扩展格式，它支持更大的文件和更多的通道数The first 4 bytes can be used to identify the container. For RIFF files it will start with "RIFF" and for
    // w64 it will start with "riff".
    //drwav__fourcc_equal函数是用来判断两个四字节码是否相等，在这里是用来判断输入的文件是RIFF格式还是w64格式
    if (drwav__fourcc_equal(riff, "RIFF")) {
        pWav->container = drwav_container_riff;
        printf("文件格式为RIFF\n");/////////////////////////////////////////////////////////////
    } else if (drwav__fourcc_equal(riff, "riff")) {
        pWav->container = drwav_container_w64;//W64的格式
        printf("文件格式为riff W64格式\n");///////////////////////////////////////////////////////
        // 判断guid的正确性 Check the rest of the GUID for validity.
        drwav_uint8 riff2[12];//24个字节，12个元素的数组,sizeof(riff2)=12？？
    ...
    //检查 WAV 文件的格式是否正确，并计算出数据块的起始位置由pWav->dataChunkDataPos记录，是第几个字节的单位
    if (pWav->container == drwav_container_riff) {
        // RIFF/WAVE
        unsigned char chunkSizeBytes[4];//解析 RIFF 格式块头部信息，读取数据块大小字段，该字段占据 4 个字节，存储在 chunkSizeBytes 数组中。
        ...
        pWav->dataChunkDataPos = 4 + sizeof(chunkSizeBytes) + sizeof(wave);//12字节
    } else {
        // W64
        unsigned char chunkSize[8];//数据块大小8字节
        if (onRead(pUserData, chunkSize, sizeof(chunkSize)) != sizeof(chunkSize)) {
            return DRWAV_FALSE;
        }
        //将8字节转换为u64的数，至少80才是正确的格式
        if (drwav__bytes_to_u64(chunkSize) < 80) {
            return DRWAV_FALSE;
        }

        drwav_uint8 wave[16];//16个字节
        if (onRead(pUserData, wave, sizeof(wave)) != sizeof(wave)) {
            return DRWAV_FALSE;
        }
        ...
        pWav->dataChunkDataPos = 16 + sizeof(chunkSize) + sizeof(wave);//40字节
    }


    // 接下来的24个字节是格式字节The next 24 bytes should be the "fmt " chunk.
    drwav_fmt fmt;
    ...
    unsigned short translatedFormatTag = fmt.formatTag;//=1,格式转换标识符，formatTag表示音频流的编码格式标识符，如PCM编码、ADPCM编码等。
    if (translatedFormatTag == DR_WAVE_FORMAT_EXTENSIBLE) {//表示使用了WAV格式扩展机制，则需要进入此判断语句进行转换类型
        translatedFormatTag = drwav__bytes_to_u16(fmt.subFormat + 0);
    }


    drwav_uint64 sampleCountFromFactChunk = 0;//计数？

    // 不能简单地假设 "data" 数据块一定是在下一个数据块。需要循环读取数据块来找到所需的 "data" 数据块The next chunk we care about is the "data" chunk. This is not necessarily the next chunk so we'll need to loop.
    drwav_uint64 dataSize;//数据大小
    for (;;)
    {
        drwav__chunk_header header;
        if (!drwav__read_chunk_header(onRead, pUserData, pWav->container, &pWav->dataChunkDataPos, &header)) {
            return DRWAV_FALSE;//判断语句中完成了找到块的格式（四字节）和后面四字节的信息并记录了date块的格式和大小
        }

        dataSize = header.sizeInBytes;//循环到的块的大小
        //找到了date数据块就跳出循环
        if (pWav->container == drwav_container_riff) {
        ...
        // 检查是否存在optional chunks，包括fact，cue，smpl块，其中fact块存储了样本的实际数量 Optional. Get the total sample count from the FACT chunk. This is useful for compressed formats.
        if (pWav->container == drwav_container_riff) {
            if (drwav__fourcc_equal(header.id.fourcc, "fact")) {
                drwav_uint32 sampleCount;//如果块的标识符是fact则再读取四字节得到样本的实际数量
                if (onRead(pUserData, &sampleCount, 4) != 4) {
                    return DRWAV_FALSE;
                }
                pWav->dataChunkDataPos += 4;//更新位置
                dataSize -= 4;//在数据大小上减去
        ...
        // 走到这说明没有数据块，则跳过当前块并更新位置Make sure we seek past the padding.
    ...
    // 此时我们找到了DATe块并开始解析原始音频数据流了，当前所在的位置已经到了原始音频数据的第一个字节处At this point we should be sitting on the first byte of the raw audio data.
    //记录原始音频数据的一些参数
    pWav->onRead              = onRead;
    pWav->onSeek              = onSeek;
    ...
    // 判断给定的 formatTag 是否是压缩格式的音频数据。是则进入判断语句The way we calculate the bytes per sample does not make sense for compressed formats so we just set it to 0.
...
drwav_uint64 drwav_read(drwav* pWav, drwav_uint64 samplesToRead, void* pBufferOut)
{
    ...
    // Cannot use this function for compressed formats.不能将此函数用于压缩格式。
    if (drwav__is_compressed_format_tag(pWav->translatedFormatTag)) {
        return 0;
    }
...
drwav_int16* drwav__read_and_close_s16(drwav* pWav, unsigned int* channels, unsigned int* sampleRate, drwav_uint64* totalSampleCount)
{
    ...
    drwav_uint64 sampleDataSize = pWav->totalSampleCount * sizeof(drwav_int16);//样本数据的字节数
    ...
    drwav_int16* pSampleData = (drwav_int16*)DRWAV_MALLOC((size_t)sampleDataSize);    // <--样本数据的字节数 Safe cast due to the check above.
    if (pSampleData == NULL) {
        drwav_uninit(pWav);
        printf("分配内存失败");
    ...
    drwav_uint64 samplesRead = drwav_read_s16(pWav, (size_t)pWav->totalSampleCount, pSampleData);//=totalSampleCount总的样本数
    ...
    drwav_uninit(pWav);//释放 pWav
    //将采样率、通道数量和总样本数返回给调用者
...
#ifndef DR_WAV_NO_STDIO
drwav_int16* drwav_open_and_read_file_s16(const char* filename, unsigned int* channels, unsigned int* sampleRate, drwav_uint64* totalSampleCount)
{//初始化采样率，通道，总的样本数，为接下来存储文件的信息做准备
    if (sampleRate) *sampleRate = 0;
    if (channels) *channels = 0;
    if (totalSampleCount) *totalSampleCount = 0;

    drwav wav;//用于读取 WAV 文件的结构体 
    //在if判断句中完成了读取音频头文件数据包括
    if (!drwav_init_file(&wav, filename)) {
        printf("文件打开失败或者文件打开成功，头文件读取失败dr_wav3379\n");/////////////
        return NULL;
    }
    printf("音频的wav.bitsPerSample(dr_wav3382=%d)\n", wav.bitsPerSample);////////
    printf("音频的wav.channels=%d\n", wav.channels);//////////////////////////////
    printf("音频的wav.sampleRate=%d\n", wav.sampleRate);/////////////////////////
    printf("音频的wav.totalSampleCount=%d\n", wav.totalSampleCount);/////////////
    return drwav__read_and_close_s16(&wav, channels, sampleRate, totalSampleCount);
}
